...
public class OneSevenEightTest {

	@Test
	public void inserts() throws Exception {
		String path = "/home/aoeng/Study/databases/all-db/zimu/ose";
		for (int i = 1; i < 12; i++) {
			insert(path + i + ".txt");
		}
	...
	public void insert(String path) throws Exception {
		...
		String sql = "insert into tb_oneseveneight(idss,uname,pwd,md5,sha1) values(?,?,?,?,?)";
		...
		while ((str = bur.readLine()) != null) {
			...
			ps.setString(2, ss[1]);
			ps.setString(3, ss[2]);
			ps.setString(4, MD5Utils.get32BitMD5(ss[2]));
			ps.setString(5, MD5Utils.getSha1(ss[2]));
			ps.addBatch();
			if (n % 100000 == 0) {
				ps.executeBatch();
				conn.commit();
